...
function imageUploaded(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    reader.onload = () => {
      ...
      axios
        .post(`http://localhost:5000/images/${userObj.id}`, data)
        .then((res) => {
          console.log(res);
          setLoading(false);
          navigate("/");
...
export function Previews(props) {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    accept: {
      // accept all image types jpeg, png, gif, svg, webp, jpg etc
      image: [
        "image/jpeg",
        "image/png",
        "image/gif",
    ...
    <>
        ...
        {files.length > 0 ? null : (
          <div {...getRootProps({ className: "dropzone" })}>
            ...
            <button className=" hover:bg-Bluish hover:text-white  text-Bluish py-2 px-6 rounded-md border-2 border-Bluish">
              <span className="flex flex-row">Browse</span>
            </button>
          </div>
        )}
      ...
      {loading && (
        <>
          <div class="w-full bg-grey400 shadow-sm rounded-full h-2.5 mb-2 dark:bg-grey700">
            <div
              class="bg-Bluish h-2.5 rounded-full dark:bg-Bluish"
      ...
      <div className="flex flex-row justify-end ">
        <div {...getRootProps({ className: "dropzone" })}>
          <input {...getInputProps()} />
          <button className=" hover:bg-Bluish hover:text-white  text-Bluish py-2 px-6 rounded-md border-2 border-Bluish">
            <span className="flex flex-row">Add More</span>
          </button>
        </div>
        <button
          className=" bg-Bluish text-white py-2 px-6 rounded-md border-2 border-Bluish ml-5"
          onClick={() => {
            imageUploaded(
              acceptImage,
              setLoading,
        ...
        >
          <span className="flex flex-row">Upload</span>
        </button>
      </div>
    </>
  );
